...
Vercel deployment entrypoint for BOT ARENA.

The original Socket.IO transport is replaced by HTTP polling so the app can run
as a Vercel Python Function. Room state is stored in Upstash Redis when the
UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN environment variables are
configured.

The game simulation advances from elapsed time on incoming requests instead of
using an always-on background thread.

Everything remains in this file:
- Flask API
- server-authoritative simulation
- HTTP polling multiplayer transport
...
REDIS_PREFIX = "botarena:v1:room:"
redis = Redis.from_env() if Redis and os.getenv("UPSTASH_REDIS_REST_URL") and os.getenv("UPSTASH_REDIS_REST_TOKEN") else None
...
def generate_room_code():
    ...
    while True:
        ...
        if code not in rooms and not (redis and redis.exists(REDIS_PREFIX + code)):
            return code
...
def save_room(room):
    room["last_update"] = monotonic()
    if redis:
        redis.set(REDIS_PREFIX + room["code"], json.dumps(_json_safe(room)), ex=7200)
    else:
...
def load_room(code):
    ...
    if redis:
        raw = redis.get(REDIS_PREFIX + code)
        if not raw:
            return None
        return _json_restore(json.loads(raw))
...
def delete_room(code):
    rooms.pop(code, None)
    if redis:
        redis.delete(REDIS_PREFIX + str(code).upper())


def advance_room(room):
    now = monotonic()
    last = safe_float(room.get("last_update"), now)
...
def public_response(room, message=None):
    result = {"state": get_state(room)}
    if message:
...
def find_player(room, player_id):
    return room.get("players", {}).get(str(player_id))


def action_error(message, code=400):
    return jsonify({"message": message}), code


def perform_action(room, player_id, action, data):
    ...
    if action == "start_game":
        if room["mode"] == "ffa" and len(room["players"]) < 3:
            return False, "FFA requires at least 3 players."
        if room["started"]:
            return True, None
        room["started"] = True
        ...
        room["wave"] = 0
        room["intermission"] = 0
        room["enemies"].clear(); room["bullets"].clear(); room["grenades"].clear(); room["pickups"].clear()
        room["announcement"] = "GET READY"; room["announcement_timer"] = 2.5
        return True, None

    if action == "move":
        dx=clamp(safe_float(data.get("x")),-1,1); dy=clamp(safe_float(data.get("y")),-1,1)
        length=math.hypot(dx,dy)
        if length>1: dx/=length; dy/=length
        player["move_dx"]=dx; player["move_dy"]=dy
    elif action == "shoot":
        fire_weapon(room, player, safe_float(data.get("angle")))
    elif action == "reload":
        if player["alive"]: player["ammo"]=player["magazine"]
    elif action == "dash":
        use_dash(player, safe_float(data.get("angle")))
    elif action == "grenade":
        use_grenade(room, player, safe_float(data.get("x"),player["x"]), safe_float(data.get("y"),player["y"]))
    elif action == "buy_item":
        return buy_item(player,str(data.get("item","")))
    elif action == "buy_weapon":
        return buy_weapon(player,str(data.get("weapon","")))
    elif action == "shop_toggle":
        if room["started"] and room["mode"] == "solo":
            opening=bool(data.get("open")); room["paused"]=opening; room["shop_paused_by"]=player_id if opening else None
    elif action == "revive":
        if player["alive"]: return False,"You are already alive."
        cost=1000*(2**player.get("revives",0))
        if player["coins"]<cost: return False,f"You need {cost:,} coins to revive."
        player["coins"]-=cost; player["revives"]=player.get("revives",0)+1
        player["alive"]=True; player["health"]=player["max_health"]; player["hit_flash"]=0
        player["x"]=random.uniform(180,WORLD_W-180); player["y"]=random.uniform(140,WORLD_H-140)
        room["squad_eliminated"]=False; room["game_over"]=False
        return True,f"{player['name']} revived for {cost:,} coins!"
    elif action == "ffa_attack":
        ffa_attack(room,player,str(data.get("target","")))
    elif action == "leave_game":
        room["players"].pop(str(player_id),None)
        if not room["players"]: return True,"__DELETE__"
    else:
...
@app.get("/api/health")
def health():
    return jsonify({"ok":True,"storage":"upstash" if redis else "memory","transport":"http-polling"})
...
def state_route():
    ...
    room=load_room(code)
    if not room:return action_error("Room not found.",404)
    with lock:
        advance_room(room); save_room(room)
        return jsonify({"state":get_state(room)})
...
def create_route():
    ...
    player_id=str(data.get("player_id") or uid())
    name=clean_name(data.get("name","Player"))
    with lock:
        # Generate against Redis too, then create and save.
        room=create_room(mode)
        room["players"][player_id]=create_player(player_id,name,0)
        save_room(room)
    return jsonify({"code":room["code"],"player_id":player_id,"state":get_state(room)})


@app.post("/api/join")
def join_route():
    data=request.get_json(silent=True) or {}
    code=str(data.get("code","")).strip().upper(); player_id=str(data.get("player_id") or uid())
    room=load_room(code)
    if not room:return action_error("Room not found.",404)
    with lock:
        advance_room(room)
        if room["started"]:return action_error("That game has already started.")
        if len(room["players"])>=MAX_PLAYERS:return action_error("That room is full.")
        room["players"][player_id]=create_player(player_id,clean_name(data.get("name","Player")),len(room["players"]))
        save_room(room)
    return jsonify({"code":code,"player_id":player_id,"state":get_state(room)})


@app.post("/api/action")
def action_route():
    data=request.get_json(silent=True) or {}
    code=str(data.get("code","")).strip().upper(); player_id=str(data.get("player_id", "")); action=str(data.get("action","")); payload=data.get("data") or {}
    room=load_room(code)
    if not room:return action_error("Room not found.",404)
    with lock:
        advance_room(room)
        ok,message=perform_action(room,player_id,action,payload)
        if not ok:return action_error(message or "Action failed.")
        if message=="__DELETE__":
            delete_room(code); return jsonify({"state":{}})
        save_room(room)
        return public_response(room,message)
...
let state = null;
let myId = localStorage.getItem("botArenaPlayerId") || crypto.randomUUID();
localStorage.setItem("botArenaPlayerId", myId);
let roomCode = localStorage.getItem("botArenaRoom") || "";
...
async function api(path, payload=null, method="POST"){
  ...
  const d=await r.json().catch(()=>({message:"Server returned invalid JSON."}));
...
async function sendAction(action,data={}){
  ...
  try{
    const d=await api("/action",{action,code:roomCode,player_id:myId,data});
...
async function poll(){
  ...
  pollBusy=true;
  try{
    const d=await api(`/state?code=${encodeURIComponent(roomCode)}&player_id=${encodeURIComponent(myId)}`,null,"GET");
    if(d.state)applyState(d.state);
  }catch(e){
    if(state) document.getElementById("error").textContent=e.message;
...
async function createGame(mode){
  ...
  try{
    const d=await api("/create",{mode,name:nameValue(),player_id:myId});
    roomCode=d.code; localStorage.setItem("botArenaRoom",roomCode);
    applyState(d.state);
...
async function joinGame(){
  ...
  try{
    ...
    const d=await api("/join",{code,name:nameValue(),player_id:myId});
    roomCode=code; localStorage.setItem("botArenaRoom",roomCode);
    applyState(d.state);
    document.getElementById("roomCode").textContent=roomCode;
  }catch(e){document.getElementById("error").textContent=e.message;}
}

async function startGame(){await sendAction("start_game");}
...
function shoot(){
  ...
  lastShot=t;
  sendAction("shoot",{angle:aimAngle()});
}

function dash(){
  if(!state)return;
  if(!document.getElementById("shop").classList.contains("hidden") && state.mode==="solo")return;
  sendAction("dash",{angle:aimAngle()});
}
...
function throwGrenade(){
  ...
  if(!document.getElementById("shop").classList.contains("hidden") && state.mode==="solo")return;
  const p=worldPoint();
  sendAction("grenade",{x:p.x,y:p.y});
}
...
addEventListener("beforeunload",()=>{navigator.sendBeacon(API+"/action",new Blob([JSON.stringify({action:"shop_toggle",code:roomCode,player_id:myId,data:{open:false}})],{type:"application/json"}))});
...
function openShop(){
  ...
  renderShop(shopTab);
  sendAction("shop_toggle",{open:true});
}

function closeShop(){
  document.getElementById("shop").classList.add("hidden");
  sendAction("shop_toggle",{open:false});
}
...
function buyItem(id){sendAction("buy_item",{item:id})}
function buyWeapon(id){sendAction("buy_weapon",{weapon:id})}
function revive(){sendAction("revive")}
...
document.addEventListener("click",e=>{
  if(e.target?.id==="reviveBtn") sendAction("revive");
  if(e.target?.id==="reviveWaitBtn") document.getElementById("reviveOverlay")?.classList.add("hidden");
  if(e.target?.id==="homeAfterWipe"){
    sendAction("leave_game");
    localStorage.removeItem("botArenaRoom");
    roomCode=""; state=null;
